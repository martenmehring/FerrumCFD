...
mod tests {
    ...
    fn large_finite_simple_update_rolls_back() {
        ...
        for (actual, initial) in report.final_velocity.iter().zip(initial_velocity) {
            assert_eq!(actual.x.to_bits(), initial.x.to_bits());
            assert_eq!(actual.y.to_bits(), initial.y.to_bits());
            assert_eq!(actual.z.to_bits(), initial.z.to_bits());
        }
        assert_eq!(report.final_pressure, [1.0, 0.0]);
    ...
    fn repeated_growing_and_sign_reversing_finite_updates_roll_back() {
        ...
        assert_eq!(report.simple_iterations, 2);
        assert_eq!(
            report.stop_reason,
            LaminarSimpleStopReason::SolverInvalidState
        );
        ...
        assert!(report.history[0].pressure_correction_accepted);
        assert!(!report.history[1].pressure_correction_accepted);
        assert_eq!(report.history[1].pressure_correction_update_scale, 0.0);
        assert!(report.history[0].relative_pressure_change_l2.is_finite());
        assert_eq!(
            report.history[1].relative_pressure_change_l2.to_bits(),
            0.0_f64.to_bits()
        );
        let first_forced = forced_solutions[0];
        let mut expected_pressure = [1.0, 0.0];
        for (value, target) in expected_pressure.iter_mut().zip(first_forced) {
