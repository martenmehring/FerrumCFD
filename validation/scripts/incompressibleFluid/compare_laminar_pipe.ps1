...
function Invoke-FerrumPreflight([string]$CaseRoot, [string]$PlanJson) {
    ...
    $exe = Join-Path $RepoRoot "target\debug\ferrumRun.exe"
    if (Test-Path -LiteralPath $exe) {
        $command = $exe
        $arguments = @("-solver", "incompressibleFluid", "-case", $CaseRoot, "--runnerDryRun", "--maxRunnerSteps", "2", "--planJson", $PlanJson)
    } else {
        $command = "cargo"
        $arguments = @("run", "--manifest-path", (Join-Path $RepoRoot "Cargo.toml"), "-p", "ferrum-run", "--bin", "ferrumRun", "--", "-solver", "incompressibleFluid", "-case", $CaseRoot, "--runnerDryRun", "--maxRunnerSteps", "2", "--planJson", $PlanJson)
    }
...
function Invoke-FerrumPoiseuilleSolve(
    ...
    $exe = Join-Path $RepoRoot "target\debug\ferrum.exe"
    if (Test-Path -LiteralPath $exe) {
        $command = $exe
        $arguments = @(
            "solve",
        ...
        $command = "cargo"
        $arguments = @(
            "run", "--manifest-path", (Join-Path $RepoRoot "Cargo.toml"), "-p", "ferrum-cli", "--bin", "ferrum", "--", "solve",
            "-case", $CaseRoot,
            "--solvePoiseuille",
            "--pressureDrop", ([double]$Physics.analyticDeltaPPa).ToString("G17", $invariant),
            "--mu", ([double]$Physics.dynamicViscosityPaS).ToString("G17", $invariant),
...
function Invoke-FerrumLaminarSimpleSolve(
    ...
    if (Test-Path -LiteralPath $exe) {
        ...
        $command = "cargo"
        $arguments = @(
            "run", "--manifest-path", (Join-Path $RepoRoot "Cargo.toml"), "-p", "ferrum-run", "--bin", "ferrumRun", "--",
            "-solver", "incompressibleFluid",
            "-case", $CaseRoot,
            "--minSimpleIterations", $SimpleIterations.ToString([System.Globalization.CultureInfo]::InvariantCulture),
            "--maxSimpleIterations", $SimpleIterations.ToString([System.Globalization.CultureInfo]::InvariantCulture),
    ...
    if (Test-Path -LiteralPath $benchmarkExe) {
        ...
        $benchmarkArguments = @("run", "--manifest-path", (Join-Path $RepoRoot "Cargo.toml"), "-p", "ferrum-cli", "--bin", "ferrumPipeBenchmark", "--") + $benchmarkArguments
    }
...
function Write-MarkdownReport($Path, $Result) {
    ...
    $ferrumRunLabel = if ($null -ne $ferrumSolve -and $ferrumSolve.mode -eq "laminar-simple") { "incompressibleFluid SIMPLE" } else { "Poiseuille utility" }
    ...
    $lines.Add("| Ferrum $ferrumRunLabel solve | $($status.ferrumRunComparison) |")
    ...
    if ($null -ne $ferrumSolve) {
        $lines.Add("| Ferrum $ferrumRunLabel solve | $(Format-NullableNumber $comparison.timing.ferrumCommandWallClockSeconds "G6") | $(Format-NullableNumber $comparison.timing.ferrumSolveWallClockSeconds "G6") | $($ferrumSolve.iterations) iterations |")
    } else {
        $lines.Add("| Ferrum $ferrumRunLabel solve | n/a | n/a | n/a |")
    }
    if ($null -ne $openFoam) {
...
if (!$SkipFerrumSolve) {
    ...
    if ($FerrumMode -eq "incompressibleFluid") {
...
$ferrumRunStatus = if ($SkipFerrumSolve) {
    "skipped"
...
$matchedRunBudget = [ordered]@{
    mode = if ($FerrumMode -eq "incompressibleFluid") { "steady-simple-pseudo-time" } else { "not-applicable" }
...
$result = [ordered]@{
    ...
    comparison = [ordered]@{
        ...
        ferrumRunComparison = $ferrumRunStatus
        timing = [ordered]@{
    ...
    benchmarkStatus = [ordered]@{
        ...
        ferrumRunComparison = $ferrumRunStatus
        readyForCiGate = ($ferrumRunStatus -eq "passed" -and $openFoamReferenceStatus -eq "passed")
        notes = $notes
    }
}
