...
cargo run --manifest-path (Join-Path $RepoRoot "Cargo.toml") -p ferrum-cli --bin gmshToFerrum -- $MeshFile -case $CaseRoot
if ($LASTEXITCODE -ne 0) {
    throw "gmshToFerrum failed with exit code $LASTEXITCODE"
}

cargo run --manifest-path (Join-Path $RepoRoot "Cargo.toml") -p ferrum-cli --bin checkFerrumMesh -- -case $CaseRoot
if ($LASTEXITCODE -ne 0) {
