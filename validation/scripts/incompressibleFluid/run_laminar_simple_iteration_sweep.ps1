...
function Get-FerrumCommand([string]$Binary) {
    ...
    $package = if ($Binary -eq "ferrumRun") { "ferrum-run" } else { "ferrum-cli" }
    return [pscustomobject][ordered]@{
        command = "cargo"
        prefix = @("run", "--manifest-path", (Join-Path $RepoRoot "Cargo.toml"), "-p", $package, "--bin", $Binary, "--")
    }
}
...
function Invoke-LaminarSimpleIterationRun(
    ...
    if ($null -eq $report) {
        ...
        $solver = Get-FerrumCommand "ferrumRun"
        $arguments = New-Object System.Collections.Generic.List[string]
