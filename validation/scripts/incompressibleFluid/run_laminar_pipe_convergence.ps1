...
function Write-StudyMarkdown($Path, $Rows, $Summary) {
    ...
    $lines.Add("## Notes")
    $lines.Add("")
    $lines.Add('- `medium` matches the versioned `tutorials/incompressibleFluid/laminarPipe/ferrum/case` default mesh resolution.')
...
foreach ($variant in $variants) {
    ...
    $rows.Add([pscustomobject][ordered]@{
            ...
            ferrum = [pscustomobject][ordered]@{
                status = if ($null -ne $comparison) { $comparison.benchmarkStatus.ferrumRunComparison } else { "missing" }
