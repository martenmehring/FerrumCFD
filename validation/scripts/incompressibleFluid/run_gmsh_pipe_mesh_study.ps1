...
if ([string]::IsNullOrWhiteSpace($GeoFile)) {
    $GeoFile = Join-Path $RepoRoot "tutorials\incompressibleFluid\laminarPipe\shared\geometry\pipe_prism2.geo"
}
...
function Invoke-FerrumCommand([string]$BinName, [string[]]$Arguments, [string]$LogPath) {
    ...
    if (Test-Path -LiteralPath $exe) {
        ...
        $command = "cargo"
        $commandArgs = @("run", "--manifest-path", (Join-Path $RepoRoot "Cargo.toml"), "-p", "ferrum-cli", "--bin", $BinName, "--") + $Arguments
    }
...
function Write-StudyMarkdown($Path, $Rows, $Summary) {
    ...
    $lines.Add("- This historical path runs the source-driven Poiseuille benchmark; current pressure-velocity validation uses the separate laminar SIMPLE mesh study.")
...
$sourceSystem = Join-Path $RepoRoot "tutorials\incompressibleFluid\laminarPipe\ferrum\case\system"
